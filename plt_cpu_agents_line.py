...
'''Plot and save WIMA vs Baseline cpu line graph (agents included).'''
    ...
    for line in open_file('timestamps.txt').splitlines():
        ...
        if wima:
            ...
            for container in containers:
                url = URL.replace('container_name', container)
                resp = requests.get(url + line, timeout=15).text
                agent_data = resp.splitlines()[1:]
                agent_data = [i.split(',')[1] for i in agent_data]
                agent_data = [float(i) for i in agent_data]
                baseline_data.append(agent_data)
            data = [sum(i) for i in zip(*baseline_data)]
